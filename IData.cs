...
{    
    ...
    private static double[] ParseCSV2DoublePrivate_Function(string[][] csvData_StringArray2D, int columnNumber_Int)
    {
        ...
        for (int row_Int = 1; row_Int < csvData_StringArray2D.Length-1; row_Int++)
        {

            try
            {
                
                targetColumn_StringArray[row_Int-1] = double.Parse(csvData_StringArray2D[row_Int][columnNumber_Int]);
                
            }
            catch (System.Exception parsing_Exception)
            {
                
                System.Console.WriteLine(parsing_Exception);
    ...
    private static string[][] GetDataCSVPrivate_Function(string filePath_String)
    {
        ...
        try
        {
            ...
            int length_Int = File.ReadAllLines(filePath_String).Length;

            string[][] csvData_StringArray2D = new string[length_Int][];

            for (int row_Int = 0; row_Int < length_Int-1; row_Int++)
            {

                csvData_StringArray2D[row_Int] = File.ReadAllLines(filePath_String)[row_Int+1].Split(',');
